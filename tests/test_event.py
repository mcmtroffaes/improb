...
def test_event_not():
    ...
    nose.tools.assert_sequence_equal(
        [c.get_value(w) for w in atoms], [True, False, True])
...
def test_func_not():
    ...
    c = b.not_()
    nose.tools.assert_sequence_equal(
        [c.get_value(w) for w in atoms], [True, False, False])

def test_event_and_or_xor():
    a = Var('abcd')
    atoms = [{a: 'a'}, {a: 'b'}, {a: 'c'}, {a: 'd'}]
    b = Func(a, [False, True, False, True])
    c = Func(a, [True, True, False, False])
    d = b & c
    nose.tools.assert_sequence_equal(
        [d.get_value(w) for w in atoms], [False, True, False, False])
    d = b | c
    nose.tools.assert_sequence_equal(
        [d.get_value(w) for w in atoms], [True, True, False, True])
    d = b ^ c
    nose.tools.assert_sequence_equal(
        [d.get_value(w) for w in atoms], [True, False, False, True])
